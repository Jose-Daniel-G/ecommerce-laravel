    ...
    <header class="bg-blue-600">
        <x-container class="px-4 py-4">
            <div class="flex justify-between items-center space-x-8">
                <button class="text-xl md:text-3xl" x-on:click="open = true"><i
                        class="fas fa-bars text-white"></i></button>
                <h1 class="text-white">
                    <a href="/" class="inline-flex flex-col items-end">
                        <span class="text-xl md:text-3xl leadin-4 md:leading-6 font-semibold">Ecommerce</span>
                        <span class="text-xs">Tienda Online</span></a>
                </h1>
                <div class="flex-1 hidden md:block">
                    <x-input oninput="search(this.value)" class="w-full"
                        placeholder="Buscar por producto, tienda o marca"></x-input>
                </div>
                <div class="flex items-center space-flex-4 md:space-x-8">
                    <x-dropdown>
                        <x-slot name="trigger">
                            @auth
                                <button
                            ...
                            @else
                                <button class="text-xl md:text-3xl"><i class="fas fa-user text-white"></i></button>
                            @endauth
                        </x-slot>
                        <x-slot name="content">
                            @guest
                                <div class="px-4 py-2">
                                    <div class="flex justify-center" class="btn btn-blue">
                                        <a href="{{ route('login') }}">Iniciar sesion</a>
                                    </div>
                                    <p class="text-sm text-center mt-2">No tienes cuenta?<a href="{{ route('register') }}"
                                            class="text-blue-600 hover:underline">Registrate</a></p>
                                </div>
                            @else
                    ...
                    <a href="{{route('cart.index')}}" class="relative"><i
                            class="fas fa-shopping-cart text-white text-xl md:text-3xl "></i>
                        <span id="cart-count"
                            class="absolute -top-2 -end-4 inline-flex items-center w-6 h-6 justify-center bg-red-500 rounded-full text-xs font-bold text-white">{{Cart::instance('shopping')->count()}}</span>
                    </a>
            ...
            <div class="w-screen md:w-80 h-screen bg-white">
                <div class="px-4 py-3 bg-blue-600 text-white font-semibold">
                    <div class="flex justify-between items-center">
                        ...
                        <button x-show="open" x-on:click="open = false"><i class="fas fa-times"></i></button>
                    </div>
                </div>
            ...
            <div class="w-80 xl:w-[57rem] pt-[52px] hidden md:block">
                <div class="bg-white h-[calc(100vh-52)] overflow-auto px-6 py-8">
                    <div class="mb-8 flex justify-between items-center">
                        <p class="border-b-[3px] border-lime-400 uppercase textxl font-semibold pb-1">
                            {{ $this->familyName }}</p>
